...
path = r'C:/aics_deployment/text_projects/ICICI_Email_Analytics/ICICI_Email_Analytics'


fp = open(path + '/TrainingData.txt', 'r')

s = fp.readlines()
training_data = []
for i in range(0, len(s)):
    stem_txt = stemSentence(s[i])
    stop_w = rm_stopword(stem_txt)
    rm_punc = strip_punctuation(stop_w)  # New Function added
    splited_val = stop_w.split(',')
    classname = splited_val[1]
    sentenc = str(splited_val[0]).lower()
    classname = classname.rstrip()
    training_data.append({'class': classname, 'sentence': sentenc})
...
def train(X, y, hidden_neurons=10, alpha=1, epochs=50000, dropout=False, dropout_percent=0.5):
    ...
    print("Synapse Shape :", synapse_0.shape)
    # persist synapses
...
with open("words", "wb") as wp:  # Pickling
    pickle.dump(words, wp)

with open("classes", "wb") as cp:
    pickle.dump(classes, cp)
